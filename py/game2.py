...
creditos_img = pygame.image.load("Recursos/Sprites/interfaz/creditos/creditos.png").convert_alpha()
...
def obtener_tile(tileset, tile_id, tile_ancho, tile_alto, columnas_tileset):
    """Extrae el tile (subsurface) a partir del ID y la organización del tileset."""
...
def cargar_mapa_csv(ruta_csv):
    """Carga el CSV y devuelve una matriz (lista de listas) de IDs de tiles."""
    mapa = []
...
def dibujar_mapa(screen, mapa, tileset, tile_ancho, tile_alto, columnas_tileset):
    """
    Recorre la matriz 'mapa' y para cada ID distinto de 0 (que asume vacío)
    obtiene y dibuja el tile correspondiente.
    """
...
# Inicializa el mixer si no lo has hecho ya
pygame.mixer.init()

def inicio_sonido():
    pygame.mixer.music.load("Recursos//Sonidos//pantalla_de_inicio//retro-synth-loop-128-41048.mp3") 
    pygame.mixer.music.set_volume(0.5)  # 0.5 es un buen valor
    pygame.mixer.music.play(-1)
    
def cursor():
    sonido_cursor = pygame.mixer.Sound("Recursos//Sonidos//pantalla_de_inicio//1or2players//arcade-ui-18-229517.mp3")
    sonido_cursor.set_volume(0.5)
    sonido_cursor.play()
    
def tema_nvl():
    pygame.mixer.music.load("Recursos//Sonidos//tema_primer_stage//game-over-groove-306764.mp3")
    pygame.mixer.music.set_volume(0.3)
    pygame.mixer.music.play(-1)
    
def explocion():
    sonido_explosion = pygame.mixer.Sound("Recursos//Sonidos//death_personajes//8-bit-explosion-95847.mp3")
    sonido_explosion.set_volume(0.5)
    sonido_explosion.play()
    
def creditos():
    pygame.mixer.music.load("Recursos//Sonidos//Victoria//brass-fanfare-with-timpani-and-winchimes-reverberated-146260.mp3")
    pygame.mixer.music.set_volume(0.5)  
    pygame.mixer.music.play(-1)
    
def balas():
    sonido_balas = pygame.mixer.Sound("Recursos//Sonidos//shoot//shoot-6-81136.mp3")
    sonido_balas.set_volume(0.5)
    sonido_balas.play()
def shootledaunenemigo():
    # Carga y reproducir el efecto usando pygame.mixer.Sound
    sonido_recibir_disparo = pygame.mixer.Sound("Recursos//Sonidos//Shootledaaunenemigo//391660__jeckkech__projectile.wav")
    sonido_recibir_disparo.set_volume(0.5)
    sonido_recibir_disparo.play()
def power_up_sonido():
    # Carga y reproducir el efecto usando pygame.mixer.Sound
    sonido_recibir_disparo = pygame.mixer.Sound("Recursos//Sonidos//power_up//retro-video-game-coin-pickup-38299.mp3")
    sonido_recibir_disparo.set_volume(0.5)
    sonido_recibir_disparo.play()
def explosion_enemigo():
    # Carga y reproducir el efecto usando pygame.mixer.Sound
    explosion_enemigo_bala = pygame.mixer.Sound("Recursos//Sonidos//death_personajes/retro-game-sfx-explosion-104422.mp3")
    explosion_enemigo_bala.set_volume(0.5)
    explosion_enemigo_bala.play()
def game_over_sonido():
    pygame.mixer.music.load("Recursos//Sonidos//game_over//videogame-death-sound-43894.mp3")
    pygame.mixer.music.set_volume(0.5)  
    pygame.mixer.music.play(1)
...
def boss_sonido():
    global boss_channel
    sonido_boss = pygame.mixer.Sound("Recursos//Sonidos//bossfinal//boom-sfx-318827.mp3")
    sonido_boss.set_volume(0.5)
    # Reproduce el sonido en bucle y guarda el canal donde se está reproduciendo
...
sprites_disparo = {
    ...
    "bala_2": escalar_img(pygame.image.load("Recursos/Sprites/Disparos/disparo_basico_S.png").convert_alpha(), 4),
    "bala_3": escalar_img(pygame.image.load("Recursos/Sprites/Disparos/disparo_basico_S.png").convert_alpha(), 1.5)
}
...
powerup_resources = {
    "R": escalar_img(pygame.image.load("Recursos/Sprites/powerups/R.png").convert_alpha(), 1.5),
    "S": escalar_img(pygame.image.load("Recursos/Sprites/powerups/S.png").convert_alpha(), 1.5),
}
...
controles = pygame.image.load("Recursos//letra//controles.jpg")
#Botones 
...
def pantalla_victoria():
    ...
    font_pequeno = pygame.font.Font("Recursos//letra//PressStart2P-Regular.ttf", 40)
    ...
    while esperando:
        ...
        screen.blit(mensaje, (constantes.ANCHO // 2 - mensaje.get_width() // 2,
                              constantes.ALTO // 2 - mensaje.get_height() // 2 - 30))
        screen.blit(reiniciar_texto, (constantes.ANCHO // 2 - reiniciar_texto.get_width() // 2,
                                      constantes.ALTO // 2 + 40))
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_r:
                    return  # Sale de la función para reiniciar el juego       
     


# Clase Boss (la que me pasaste)
    ...
    def actualizar_explosion(self):
        ...
        if now - self.explosion_update_time > self.explosion_cooldown:
            ...
            if self.explosion_index < len(self.explosion_frames):
                # Escalamos la imagen para que la explosión se vea más grande (factor de 2, por ejemplo)
                ...
                # Si no se ha registrado aún el tiempo de término de la explosión, lo registramos.
                if not hasattr(self, "explosion_finish_time"):
                    self.explosion_finish_time = now
                # Esperamos 2000 ms adicionales (puedes ajustar el valor) antes de terminar
...
class DisparoEnemigo(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        # Suponiendo que scroll_x es una variable global que representa el desplazamiento actual
        global scroll_x  
        # Calculamos la posición de la bala en la pantalla
        pantalla_x = self.rect.x - scroll_x  
        # Verificamos si la bala está fuera de la pantalla (asumiendo 1280x720)
...
def juego():
   ...
   # Cargar y escalar la imagen del disparo enemigo
    ...
    # Crear al boss
    boss = Boss(
        position=(10000, 400),  # Posición temporal para probar
        health=10,
    ...
    while run:
        ...
        if boss_activo:
            # Si el sonido del boss aún no se ha reproducido, se reproduce y se marca la bandera
            ...
            # Aquí, si deseas que se reinicie la posibilidad de reproducir el sonido cuando se salga de la fase (opcional)
            boss_sound_played = False
            # Lógica de desplazamiento normal basada en el jugador
        ...
        # Supongamos que 'disparos' es el grupo de balas del jugador
        for bala in list(disparos):
            # Puedes ajustar la colisión si usas scroll o coordenadas del mundo
            if bala.rect.colliderect(boss.rect):
                boss.recibir_daño(bala.daño)  # Asegúrate de que la bala tenga su atributo 'daño'
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                # Primero, evaluamos si se presionó Escape o R para regresar al inicio
                ...
                if event.type == pygame.QUIT:
                    ...
                    if boton_jugar1p.collidepoint(event.pos):
                        if boton_jugar1p.collidepoint(event.pos):
                            sonidos.cursor()
                            fade(screen)
                            tema_nvl()            # Llamamos una sola vez al cambiar de estado
                            estado = "juego"
